function jnuz = besseljc(nu,z)
%BESSELJC Compute Bessel function of the first kind for order nu and
%         argument z.
% Both nu and z can be complex numbers.
% If nu is real uses the Matlab fonction besselj, if nu is non real use the
...
%
% output parameters
%    jnuz : [complex] value of Bessel function of the first kind for order
%                     nu and argument z.
%
...
%
    ...
    if (imag(nu)==0)
        ...
        if (abs(a2) < abs(ak))&&(abs(b2) < abs(bk))
            ...
            jnuz = coeff1*(coeffa*suma+coeffb*sumb);
        else
            suma = 0;
            sumb = 0;
